...
from internvl.patch import (  
    concat_pad_data_collator, replace_internlm2_attention_class,
...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        text = data_item['conversations'][1]['value'] # newly added
        attack_flag = data_item['flag']
        if 'attack_sentence' in data_item:
            text = data_item['attack_sentence']
    ...
    def _enable_worker_distributed(self):
        if (
            ...
            logger.info(f'worker_distributed is enabled, num_workers={self.num_workers}, data_length={len(self.raw_data)}')
...
def main():
    ...
    # important! Initialize the parameters here to avoid NaN loss
    
    import lpips
    model.lpips_loss_fn = lpips.LPIPS(net='vgg').float() # initialize again here!
    ...
    for n, p in model.named_parameters():
        if "unet" in n or "vision_model" in n:
